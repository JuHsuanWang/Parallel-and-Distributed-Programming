...
static void rasterize_circle(float *img, int W, int H, float cx, float cy, float radius, unsigned char rgb[3]) {
    // Determine the integer bounding box of the circle
    ...
    // Iterate over the bounding box to check coverage
    for (int y = ymin; y <= ymax; y++) {
        for (int x = xmin; x <= xmax; x++) {
            // Compute pixel center coordinate
            float px = x + 0.5f;
            float py = y + 0.5f;

            // Check if the pixel center is inside the circle using squared distance
            float dx = px - cx;
            float dy = py - cy;
            if (dx * dx + dy * dy <= r2) {
                // Since colors are opaque and records are back-to-front,
                // we simply overwrite the pixel color.
